...
def _strip_html(html_content: str) -> str:
    ...
    # List items: add bullet/number hint before content
    text = re.sub(r'<li[^>]*>', '• ', text)
    text = re.sub(r'</li>', '\n', text)
    text = re.sub(r'</?[uo]l[^>]*>', '\n', text)
    # Headings → newline after
    text = re.sub(r'</h[1-6]>', '\n', text)
    # Strip all remaining tags
    text = re.sub(r'<[^>]+>', '', text)
    # Decode HTML entities (&amp; &lt; &gt; etc.)
    text = html_module.unescape(text)
    # Replace non-breaking spaces and other whitespace variants with normal space
...
@tool
def get_team_updates(date: str) -> str:
    """Get all team updates for a given date (YYYY-MM-DD).
    Pass empty string '' to get today's updates.
...
@tool
def get_missing_updates(date: str) -> str:
    """Get list of team members who have NOT submitted an update for a given date.
    Pass empty string '' for today. Returns names and emails grouped by team."""
    target_date = date if date else str(dt_date.today())
    teams = get_all_teams()
...
@tool
def get_meeting_notes_tool(team_name: str, date: str) -> str:
    """Get the meeting MoM/notes for a team on a given date.
    Pass empty string '' for today's notes."""
    target_date = date if date else str(dt_date.today())
    teams = get_all_teams()
...
@tool
def summarize_updates(team_name: str, date: str) -> str:
    """Show the team updates for a given date as clean plain text (for display in chat).
    Includes MoM if meeting notes exist. Pass empty string '' for today.
...
@tool
def send_email_report(to_email: str, subject: str, team_name: str, date: str) -> str:
    ...
    team_name: team to send for, or 'all' for all teams. date: YYYY-MM-DD or '' for today."""
    target_date = date if date else str(dt_date.today())
    all_teams = get_all_teams()

    # Build body internally so unicode/newlines are never passed through LLM JSON args
    body = _build_email_body(team_name, target_date)
    if body is None:
        return (
            f"No team found with name '{team_name}'. "
    ...
    # Deduplicate while preserving order, remove to_email from CC
    seen: set[str] = set()
    unique_cc: list[str] = []
...
- DATE RULE: Always default to today's date unless the user explicitly says "yesterday", "day before yesterday", a specific date, or asks for multiple days (e.g. "today and yesterday"). Never guess a past date. Pass '' (empty string) for today in all tool calls unless a different date is clearly requested.
...
def run_chatbot_query(user_input: str, chat_history: list, user_info: dict | None = None) -> str:
    ...
    for _ in range(5):  # max 5 tool-call rounds
        ...
        for tc in response.tool_calls:
            ...
            if tool_fn:
                # Coerce string-typed integers to int (small models sometimes pass wrong types)
    ...
    # If the model gave a vague acknowledgment without showing the data, append raw results
